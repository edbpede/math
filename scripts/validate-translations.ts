 ...
 */
function extractKeys(obj: Record<string, any>, prefix = ""): Map<string, string> {
 ...
 */
function validateJSONFormat(locale: string, component: string, filePath: string): boolean {
 ...
 */
function validateFormat(targetKeys: Map<string, string>, locale: string, component: string): void {
...
function validateLocale(locale: string, componentFilter?: string): void {
  ...
  const componentsToValidate = componentFilter ? [componentFilter] : COMPONENTS;
...
function printSummary(): void {
  ...
  if (errorCount === 0 && warningCount === 0) {
    ...
    log(
      `   Checked ${TARGET_LOCALES.length} locale(s) across ${COMPONENTS.length} component(s)\n`,
      "green",
    );
  } else {
    if (errorCount > 0) {
