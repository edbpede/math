...
async function generateIconsWithSharp() {
  try {
    ...
    for (const size of ICON_SIZES) {
      ...
      const sharpInstance = sharp.default(svgBuffer).resize(size, size, {
        fit: "contain",
...
function generateFallbackIcons() {
  ...
  for (const size of allSizes) {
    const filename =
      allSizes.includes(size) && MASKABLE_SIZES.includes(size)
    ...
    const wrappedSVG = svgContent.replace("<svg", `<svg width="${size}" height="${size}"`);
