...
import { hashObject, memoize, memoizeAsync, memoizeWithExpiry, weakMemoize } from "./memoization";
...
describe("Performance", () => {
  ...
  it("should maintain >80% cache hit rate in realistic scenario", () => {
    ...
    for (let i = 0; i < calls; i++) {
      ...
      const value =
        Math.random() < 0.8 ? Math.floor(Math.random() * 50) : Math.floor(Math.random() * 200);
      memoized(value);
    }
