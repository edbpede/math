...
import { batchAsync, batchUpdates, createBatchQueue, deferUntilIdle, untrack } from "./reactivity";
...
describe("batchAsync", () => {
  it("should process operations in batches", async () => {
    const operations = Array.from({ length: 25 }, (_, i) => vi.fn(async () => i));
  ...
  it("should handle different batch sizes", async () => {
    const operations = Array.from({ length: 15 }, (_, i) => vi.fn(async () => i * 2));
...
describe("createBatchQueue", () => {
  it("should queue items and process them together", async () => {
    const processor = vi.fn(async (_items: number[]) => {
      // Process all items
    });
