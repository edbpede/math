...
import { type ComposeSessionOptions, composeSession } from "./composer";
...
export async function startPracticeSession(
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "[SessionLifecycle] Failed to create session record, queuing for offline sync:",
        error,
      );
...
> {
  try {
    ...
    try {
      await endSessionInDB(state.session.id, completed, correct, avgTimePerExercise);
      console.log("[SessionLifecycle] Session record updated in database");
    } catch (error) {
      console.warn(
        "[SessionLifecycle] Failed to update session record, queuing for offline sync:",
        error,
      );
