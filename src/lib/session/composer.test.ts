...
function createSRSParams(
    overrides: Partial<SRSParameters> = {},
): SRSParameters {
    return {
        easeFactor: 2.5,
        interval: 1,
    ...
    it("should use provided configuration for content balance", () => {
        ...
        const skills = Array.from({ length: 20 }, (_, i) =>
            createSkillProgress({
                ...
                nextReview: new Date(
                    Date.now() - (i % 2 === 0 ? 1 : -1) * 24 * 60 * 60 * 1000,
                ), // Half due
            }),
        );
    ...
    it("should shuffle exercises to distribute categories", () => {
        const skills = Array.from({ length: 20 }, (_, i) =>
            createSkillProgress({
                ...
                nextReview: new Date(
                    Date.now() - (i % 3 === 0 ? 1 : -1) * 24 * 60 * 60 * 1000,
                ),
            }),
        );
        ...
        if (result.status === "success") {
            ...
            const firstFiveCategories = new Set(
                exercises.slice(0, 5).map((ex) => ex.category),
            );
            const lastFiveCategories = new Set(
                exercises.slice(-5).map((ex) => ex.category),
            );
        ...
        if (result.status === "success") {
            ...
            weakExercises.forEach((exercise) => {
                if (exercise.skillProgress) {
                    expect(exercise.skillProgress.masteryLevel).toBeLessThan(
                        40,
                    );
                }
            });
