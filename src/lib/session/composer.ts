...
import { type TemplateSelectionCriteria, templateRegistry } from "../exercises/template-registry";
...
export function composeSession(options: ComposeSessionOptions): SessionCompositionResult {
  try {
    ...
    const newSkills = identifyNewContent(options.skillsProgress, newContentCriteria, currentDate);

    const reviewSkills = identifyReviewContent(options.skillsProgress, currentDate);

    const weakAreaSkills = identifyWeakAreas(options.skillsProgress, weakAreaCriteria);
...
function calculateCategoryAllocation(config: SessionCompositionConfig): CategoryAllocation {
  ...
  const allocated = newCount + reviewCount + weakAreaCount + randomCount;
  const remainder = totalExercises - allocated;
...
function identifyWeakAreas(
  ...
  const weakSkills = skillsProgress.filter(
    (skill) =>
      skill.masteryLevel <= criteria.maxMasteryLevel && skill.attempts >= criteria.minAttempts,
  );
...
function selectTemplatesForCategory(
  ...
  while (exercises.length < count && consecutiveFailures < maxConsecutiveFailures) {
    ...
    const templateId = templateRegistry.select(criteria, weights, skill.masteryLevel);
