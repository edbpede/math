 ...
 */
function describeNumberLine(data: NumberLineData, t: TranslationFunction): string {
  ...
  for (const hop of data.hops) {
    if (hop > 0) {
      ...
      hopDescriptions.push(`${t('accessibility.math.operations.minus')} ${Math.abs(hop)}`)
    }
  }
 ...
 */
function describePlaceValue(data: PlaceValueData, t: TranslationFunction): string {
...
function describeFraction(data: FractionData, t: TranslationFunction): string {
  const key = data.shapeType === 'circle'
...
function describeDiagram(data: DiagramData, t: TranslationFunction): string {
  switch (data.diagramType) {
    ...
    default:
      ...
      const description = (data as unknown as { description?: string }).description
...
function describeChart(data: ChartData, t: TranslationFunction): string {
  ...
  const description = (data as unknown as { description?: string }).description || 'data visualization'
  ...
  switch (visualAid.type) {
    ...
    case 'diagram': {
      ...
      if (data.diagramType === 'place-value') {
        ...
        return `This place value diagram shows the number ${total} broken down into its components. ` +
      ...
      if (data.diagramType === 'fraction') {
        ...
        return `This fraction diagram uses a ${shapeName} divided into ${denominator} equal parts. ` +
