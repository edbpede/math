...
function describeChart(data: ChartData, t: TranslationFunction): string {
  ...
  const description =
    (data as unknown as { description?: string }).description || "data visualization";
 ...
 */
function describeImage(data: ImageData, _t: TranslationFunction): string {
 ...
 */
export function generateVisualAidDescription(visualAid: VisualAid, t: TranslationFunction): string {
  try {
    switch (visualAid.type) {
      ...
      case "image":
        return describeImage(visualAid.data as ImageData, t);
      default:
