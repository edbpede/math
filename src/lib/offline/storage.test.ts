...
describe('OfflineStorage', () => {
  ...
  describe('Sync Queue Store', () => {
    ...
    it('should get sync queue items by type', async () => {
      ...
      const exerciseItems = await storage.getSyncQueueByType('exercise_complete')
      const progressItems = await storage.getSyncQueueByType('progress_update')
      const sessionItems = await storage.getSyncQueueByType('session_end')
  ...
  describe('Progress Cache Store', () => {
    ...
    it('should detect non-expired cache', async () => {
      ...
      const isExpired = await storage.isProgressCacheExpired('competency')
    ...
    it('should handle different data types', async () => {
      ...
      expect(await storage.getValidProgressCache('competency')).toBe(stringData)
      
      await storage.setProgressCache('skills', numberData, 60)
      expect(await storage.getValidProgressCache('skills')).toBe(numberData)
      
      await storage.setProgressCache('history', objectData, 60)
      expect(await storage.getValidProgressCache('history')).toEqual(objectData)
      
      await storage.setProgressCache('competency', arrayData, 60)
      expect(await storage.getValidProgressCache('competency')).toEqual(arrayData)
    })
  })
  ...
  describe('Preferences Store', () => {
    ...
    it('should store different data types', async () => {
      ...
      expect(await storage.getPreference('object')).toEqual({ nested: 'value' })
  ...
  describe('Error Handling', () => {
    it('should have StorageError class defined', () => {
      const error = new StorageError('Test error', 'testOperation', new Error('cause'))
