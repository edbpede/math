...
import { mergeCompetencyProgress, mergeSkillProgress } from "./conflict-resolution";
import type {
...
export async function syncQueueItem(item: SyncQueueItem): Promise<void> {
  switch (item.type) {
    ...
    default: {
      ...
      throw new Error(`Unknown sync item type: ${(_exhaustiveCheck as SyncQueueItem).type}`);
    }
  }
}
 ...
 */
async function syncExerciseComplete(item: ExerciseCompleteSyncItem): Promise<void> {
...
export async function syncProgressUpdateWithConflictResolution(
  ...
  console.log("[SyncOps] Syncing progress with conflict resolution for user:", userId);
  ...
  if (localCompetency && localCompetency.length > 0) {
    ...
    for (const local of localCompetency) {
      try {
        ...
        console.warn("[SyncOps] Error fetching server competency data, using local:", error);
        mergedCompetency.push(local);
      }
    }
    ...
    console.log("[SyncOps] Competency progress synced:", mergedCompetency.length, "items");
  }
  ...
  if (localSkills && localSkills.length > 0) {
    ...
    for (const local of localSkills) {
      try {
        // Fetch current server data
        const serverData = await fetchSkillProgressBySkill(userId, local.skillId);
        ...
        console.warn("[SyncOps] Error fetching server skill data, using local:", error);
        mergedSkills.push(local);
      }
    }
    ...
    console.log("[SyncOps] Skills progress synced:", mergedSkills.length, "items");
  }
}
