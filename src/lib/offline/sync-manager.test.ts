...
const mocks = vi.hoisted(() => {
    ...
    return {
        ...
        mockAddToSyncQueue: vi
            .fn()
            .mockImplementation(async (item: Omit<SyncQueueItem, "id">) => {
                const id = nextId++;
                mockQueue.push({ ...item, id } as SyncQueueItem);
                return id;
            }),
        mockGetAllSyncQueue: vi
            .fn()
            .mockImplementation(async () => [...mockQueue]),
        mockRemoveFromSyncQueue: vi
            .fn()
            .mockImplementation(async (id: number) => {
                mockQueue = mockQueue.filter((item) => item.id !== id);
            }),
        mockIncrementSyncRetries: vi
            .fn()
            .mockImplementation(async (id: number) => {
        ...
        mockGetSyncQueueCount: vi
            .fn()
            .mockImplementation(async () => mockQueue.length),
...
describe("SyncManager", () => {
    ...
    describe("queue management", () => {
        ...
        it("should throw error when queue is full", async () => {
            const smallQueue = new SyncManager({
                maxQueueSize: 1,
                autoSync: false,
            });
            ...
            await expect(smallQueue.addToQueue(item)).rejects.toThrow(
                "Sync queue is full",
            );
