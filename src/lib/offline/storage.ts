...
export class OfflineStorage {
        ...
        try {
            this.dbPromise = openDB<OfflineDatabase>(DB_NAME, DB_VERSION, {
                upgrade(db, oldVersion, newVersion, _transaction) {
                    console.log(
                        "[Storage] Upgrading database from version",
                        oldVersion,
                        "to",
                        newVersion,
                    );
                    ...
                    if (!db.objectStoreNames.contains("exercises")) {
                        const exerciseStore = db.createObjectStore(
                            "exercises",
                            { keyPath: "instance.id" },
                        );
                ...
                blocked() {
                    console.warn(
                        "[Storage] Database upgrade blocked - close other tabs",
                    );
                },
                blocking() {
                    console.warn(
                        "[Storage] This tab is blocking a database upgrade",
                    );
                },
            });
    ...
    async getProgressCache(
        type: ProgressCacheType,
    ): Promise<ProgressCacheEntry | undefined> {
        try {
            const db = await this.getDB();
