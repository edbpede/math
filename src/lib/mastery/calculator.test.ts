...
describe("calculateMasteryLevel", () => {
  ...
  it("should weight recent attempts more heavily", () => {
    ...
    expect(improvingResult.status).not.toBe("error");
    expect(decliningResult.status).not.toBe("error");
    if (improvingResult.status !== "error" && decliningResult.status !== "error") {
...
describe("Integration Tests", () => {
  ...
  it("should calculate realistic mastery for rusty student (not practiced recently)", () => {
    ...
    if (
      ...
      (freshResult.status === "success" || freshResult.status === "insufficient-data")
    ) {
