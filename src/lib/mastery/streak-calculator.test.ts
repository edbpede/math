...
import { calculatePracticeStreak, formatStreakMessage, type StreakData } from "./streak-calculator";
...
describe("calculatePracticeStreak", () => {
  ...
  it("should handle exercises in random order", () => {
    const exercises = [createExercise(2), createExercise(0), createExercise(3), createExercise(1)];
...
describe("Edge Cases", () => {
  ...
  it("should handle very large history efficiently", () => {
    ...
    const exercises = Array.from({ length: 1000 }, (_, i) => createExercise(Math.floor(i / 3)));
