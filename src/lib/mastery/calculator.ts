...
import type { ExerciseAttempt, MasteryLevelBand, SkillProgress } from "./types";
...
export function calculateConsistencyScore(recentAttempts: ExerciseAttempt[]): number {
  ...
  // Calculate mean
  const mean = binaryResults.reduce((sum: number, val) => sum + val, 0) / binaryResults.length;

  // Calculate variance
  const variance =
    binaryResults.reduce((sum: number, val) => sum + (val - mean) ** 2, 0) / binaryResults.length;
