...
import { formatUUID, generateUUID, parseUUID, validateUUID } from "./uuid";
 ...
 */
export async function getUserByUUID(uuid: string): Promise<AuthResult<{ user: User } | null>> {
  try {
    if (!validateUUID(uuid)) {
      return {
        success: false,
...
export async function updateUser(
  ...
  try {
    const updateData = {
      ...
      ...(updates.preferences && {
        preferences: updates.preferences as Json,
      }),
      last_active_at: new Date().toISOString(),
    };
 ...
 */
export async function deleteUser(userId: string): Promise<AuthResult<{ deleted: boolean }>> {
  try {
