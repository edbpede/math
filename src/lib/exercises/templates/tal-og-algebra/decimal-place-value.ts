...
// formatNumber not currently used but available for future enhancements
...
export const decimalPlaceValueA: ExerciseTemplate = {
  ...
  generate: (params, _locale) => {
    ...
    const formattedDecimal = _locale === "da-DK" ? `0,${tenths}` : `0.${tenths}`;
...
export const decimalPlaceValueB: ExerciseTemplate = {
  ...
  generate: (params, _locale) => {
    ...
    const formattedDecimal =
      _locale === "da-DK" ? `0,${tenthsStr}${hundredthsStr}` : `0.${tenthsStr}${hundredthsStr}`;
