...
export const numberOrderingA: ExerciseTemplate = {
  ...
  generate: (params, locale) => {
    ...
    return {
      ...
      correctAnswer: {
        ...
        equivalents: [sortedNumbers.join(","), sortedNumbers.join(", "), sortedNumbers.join(" ")],
      },
    };
  },
  ...
  hints: [
    ...
    (params, locale) => {
      ...
      if (locale === "da-DK") {
        const step1 = direction === "ascending" ? `Mindste: ${sorted[0]}` : `Største: ${sorted[0]}`;
        const step2 = direction === "ascending" ? `Mellem: ${sorted[1]}` : `Mellem: ${sorted[1]}`;
        const step3 = direction === "ascending" ? `Største: ${sorted[2]}` : `Mindste: ${sorted[2]}`;
        return `${step1}\n${step2}\n${step3}`;
      }
      const step1 = direction === "ascending" ? `Smallest: ${sorted[0]}` : `Largest: ${sorted[0]}`;
      const step2 = direction === "ascending" ? `Middle: ${sorted[1]}` : `Middle: ${sorted[1]}`;
      const step3 = direction === "ascending" ? `Largest: ${sorted[2]}` : `Smallest: ${sorted[2]}`;
      return `${step1}\n${step2}\n${step3}`;
    },
...
export const numberOrderingTemplates = [numberOrderingA, numberOrderingB, numberOrderingC];
