  ...
  hints: [
    ...
    (params, locale) => {
      ...
      if (locale === 'da-DK') {
        ...
        const groups = Array.from({ length: quotient }, (_, i) => divisor * (i + 1));
...
export const divisionC: ExerciseTemplate = {
  ...
  metadata: {
    ...
    difficulty: 'C',
    isBinding: true,
    tags: ['division', 'advanced-division', 'whole-numbers', 'times-tables'],
  },
  parameters: {
    divisor: {
      type: 'integer',
    ...
    (params, locale) => {
      ...
      if (locale === 'da-DK') {
        ...
        for (let i = start; i <= end; i++) {
          const product = divisor * i;
          if (product === dividend) {
            steps.push(`${divisor} × ${i} = ${product} ← Dette matcher!`);
          } else {
            steps.push(`${divisor} × ${i} = ${product}`);
          }
        }
      ...
      for (let i = start; i <= end; i++) {
        const product = divisor * i;
        if (product === dividend) {
          steps.push(`${divisor} × ${i} = ${product} ← This matches!`);
        } else {
          steps.push(`${divisor} × ${i} = ${product}`);
        }
      }
